...
class CDcor(nn.Module):
	...
	def forward(self, x):
		...
		gp_channel = self.gp(x) #8, 64, 1, 1, 1
		gp_depth = self.gp(x.view(batch_size, D, ch, H, W))  #8, 32, 1, 1, 1

		channel_vector = torch.squeeze(torch.squeeze(torch.squeeze(gp_channel, 2), 2), 2)
		channel_vector = self.fc_channel(channel_vector)

		depth_vector = torch.squeeze(torch.squeeze(torch.squeeze(gp_depth, 2), 2), 2)
		depth_vector = self.fc_depth(depth_vector).view(batch_size, 1, D, 1, 1)

		correlation = self.correlation(channel_vector).view(batch_size, 1, D, 1, 1)
		output = depth_vector * correlation + depth_vector
		output = torch.mul(x, output) + x
		return output
