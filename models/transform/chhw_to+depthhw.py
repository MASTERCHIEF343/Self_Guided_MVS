...
class CDcor(nn.Module):
    ...
    def forward(self, x):
        ...
        gp_channel = self.gp(x) #batch_size, channel, 1, h, w
        gp_depth = self.gp(x.view(batch_size, D, ch, H, W))  #batch_size, depth, 1, h, w

        gp_channel = torch.squeeze(torch.squeeze(torch.squeeze(gp_channel, 2), 2), 2)
        gp_channel = gp_channel.view(-1, H * W, ch)
        ch_to_depth = self.correlation(gp_channel).view(-1, D, H, W)  #batch_size, depth, H, W

        gp_depth = torch.squeeze(torch.squeeze(torch.squeeze(gp_depth, 2), 2), 2)
        gp_depth = gp_depth.view(-1, H * W, D)
        gc_depth = self.fc_depth(gp_depth).view(-1, D, H, W)

        depth = gc_depth * ch_to_depth + gc_depth
        depth = depth.view(batch_size, 1, D, H, W)

        output = torch.mul(depth, x) + x
