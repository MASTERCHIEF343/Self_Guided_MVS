...
class CDcor(nn.Module):
	...
	def forward(self, x):
		...
		gp_channel = self.gp(x)
		gp_depth = self.gp(x.view(batch_size, D, ch, H, W))

		channel_vector = self.fc_channel()
		channel_vector = torch.squeeze(torch.squeeze(torch.squeeze(channel_vector, 2), 2), 2)
		depth_vector = self.fc_depth()
		depth_vector = torch.squeeze(torch.squeeze(torch.squeeze(depth_vector, 2), 3), 3)

		correlation = self.correlation(channel_vector)
		output = depth_vector * correlation + depth_vector
		output = depth_vector.view(output, 1, D, 1, 1)
...
class YunNetTest(nn.Module):
	...
	def forward(self, target_img, refs, pose, intrinsics, intrinsics_inv):
		...
		for j, ref in enumerate(refs):
			...
			cost0 = torch.mul(cost, cdcorrelation) + cost
