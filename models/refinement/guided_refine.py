...
class Guided_filter(nn.Module):
    ...
    def forward(self, guide_feature, input):
        volume = torch.cat([input, guide_feature], dim = 1)
        output = self.conv(volume)
        return output
...
class Guided_filter(nn.Module):
    ...
    def forward(self, guide_feature, input):
        batch_size, ch, d, h, w = input.size()
        guide_feature_gp = self.gp(guide_feature)
        guide_feature_gp = torch.squeeze(torch.squeeze(guide_feature_gp, 2), 2)
        input_gp = self.cv_gp(input)
        input_gp = torch.squeeze(torch.squeeze(torch.squeeze(input_gp, 2), 2), 2)
        total_gp = guide_feature_gp + input_gp

        ch_weight = self.fc(total_gp).view(batch_size, ch, 1, 1, 1)
        output = torch.mul(input, ch_weight) + input
        return output
...
class YunNet(nn.Module):
	def __init__(self, nlabel, mindepth):
        ...
        self.corre_softmax = nn.Softmax(1)
	...
	def forward(self, target_img, refs, pose, intrinsics, intrinsics_inv):
		...
		for j, ref in enumerate(refs):
			...
			for i in range(self.nlabel):
                ...
                correlation = Variable(torch.sqrt((target_fea - ref_fea_t) ** 2 )/32).cuda()
                correlation = self.corre_softmax(correlation)

				cost[:, :target_fea.size()[1], i, :,:] = target_fea * correlation + target_fea
				cost[:, target_fea.size()[1]:, i, :,:] = ref_fea_t * correlation + ref_fea_t

                cost[:, :, i, :, :] += -torch.mul(cost[:, :, i, :, :], correlation)
